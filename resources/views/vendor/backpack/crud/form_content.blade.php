    ...
    <script>
      $(document).ready(function(){
          $('select[name="account_id"]').on('change', function(){
              ...
              if(accountId) {
                  $.ajax({
                      url: '/admin/deal/account-count/' + accountId,
                      type: 'GET',
                      success: function(data) {

                          var nextNumber = data.count + 1;
                          var generatedName = accountName + ' ' + nextNumber;
                          $('input[name="deal_name"]').val(generatedName);
                      },
                      error: function(err) {
                          console.error('Error retrieving deal count', err);
                          $('input[name="deal_name"]').val(accountName + ' 1');
                      }
                  });
              } else {
                  $('input[name="deal_name"]').val('');
              }
          });
      });
    </script>
